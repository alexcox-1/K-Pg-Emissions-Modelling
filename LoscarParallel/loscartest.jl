...
emis_co2 = [[501,660,3000] [858,859,425] [858,1212,3000]];
# here ends user input
# change this to one column for emissions 08/28
emissions = zeros(2000);
times = (1:2000).*1000;

# find the years of emissions, and add it to an array
for i in 1:length(emis_co2[1,:])
    temp_length = emis_co2[2,i]+1-emis_co2[1,i]
    temp_emss = emis_co2[3,i] / (temp_length*1000);
    emissions[emis_co2[1,i]:emis_co2[2,i]] .+= temp_emss;
end
...
loscartext = """RESTART dat/y0preind-2.0.4.dat
